# load the required libraries,  already senr
library(plyr)
library(dyplr)
library(Biobase)
library(BootMRMR)
...
#*****************************************************************************************************************
# Data Reading and Setting, please skip this step as it is performed and the data is merged into finaldata.csv
#*****************************************************************************************************************
...
# append the status column to each dataframe... execute if you want to overwrite what has been saved, dont though
...
write.csv(result, file = "final.csv")

#*****************************************************************************************************************
# Filtering Step
#*****************************************************************************************************************
# MRMR feauture selection AND select top 2000 genes, threads = 0 means its parallelized
selection.result = MRMR(my.data[,1:20429], as.factor(my.data$STATUS), k = 2000, threads = 0)

# SAVE the selected probes as a csv file as this step is computational expensive
# be super careful if u overwrite the previous features.csv, you have to set the first  column name to "features"
# otherwise following steps will return null
write.csv(selection.result, file = "features2.csv")
...
ready_data$class = my.data$STATUS
...
rfdata <- ready_data[!is.na(names(ready_data))]
...
#************************************************************************************************************
# read the features from the whole dataset
ready_data = my.data[,c(as.factor(features$features))]
...
class = as.factor(ready_data[,2001])
