...
SCLC_data = subset(data, data$class == "SCLC")
Normal_data = subset(data, data$class == "NORMAL")
ADLC_data = subset(data, data$class == "ADLC")

data = rbind(Normal_data,ADLC_data )
...
complete_dend <- color_branches(hc, h = 0.5)
...
limit = max(ADLC.mean, NORMAL.mean)

# Scatter plot of the 2 subtypes
plot(ADLC.mean ~NORMAL.mean, xlab = "ADLC", ylab = "SCLC",
...
for(i in 1 : nrow(data)) { # For each gene : 
  ...
  y = NORMAL[i,] # SCLC of gene number i
...
filter_by_fold = fold <= fold_cutoff | fold >= 1.1
